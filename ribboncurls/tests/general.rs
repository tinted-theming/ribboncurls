...
fn render_with_comments() {
    let source = "<div>{{ ! some # comment }}{{text}}</div><p>An {{ fruit }}{{!-- A comment with symbols |}# --}} is a good fruit.</p>";
    let data = r#"
        text: Some content
        fruit: orange"#;
    let rendered = ribboncurls::render(source, data, None).unwrap();
...
fn renders_with_empty_data() {
    let source = "This is pretty silly, but why not?";
    let rendered = ribboncurls::render(source, "", None).unwrap();
...
fn can_render_sections_from_bool() {
    let source = "Hello!{{#secret}} This is a secret!{{/secret}}";

    let data_show = "secret: true";
    let data_hide = "secret: false";
    let template_show = ribboncurls::render(source, data_show, None).unwrap();
    let template_hide = ribboncurls::render(source, data_hide, None).unwrap();
...
#[test]
fn render_inverse_sections() {
    let source = "Hello!{{^secret}} This is NOT a secret!{{/secret}}";
    let data_show = "secret: true";
    let data_hide = "secret: false";
    let template_show = ribboncurls::render(source, data_show, None).unwrap();
    let template_hide = ribboncurls::render(source, data_hide, None).unwrap();
...
fn can_render_inverse_sections_for_empty_strs() {
    let source = "Hello {{name}}{{^name}}Anonymous{{/name}}!";
    let data_named = "name: Maciej";
    let data_unnamed = r#"name: """#;
    let template_named = ribboncurls::render(source, data_named, None).unwrap();
    let template_unnamed = ribboncurls::render(source, data_unnamed, None).unwrap();
...
#[test]
fn render_contextual_data_in_sequence() {
    let source = "{{name}}:{{#fruits}} {{name}}{{/fruits}}";
    let data = r#"
        ...
        fruits:
            ...
            - name: banana
        "#;
    let template = ribboncurls::render(source, data, None).unwrap();

    assert_eq!(template, "Some fruits are: orange banana");
}

#[test]
fn render_contextual_data_from_parents() {
    let source = "{{#parent}}{{child.name}}'s parent is {{parent}}.{{/parent}}";
    let data = r#"
        parent: Theming
        child: 
          name: Tinted
...
// #[test]
// fn data_with_many_sections() {
//     let source = "{{#a}}{{#b}}{{#c}}{{.}}{{/c}}{{/b}}{{/a}}";
//     let data = r#"
//         a: true
//         b: 0
//         c: Tinted Theming
